...
const OnboardingScreen = ({ user, onComplete }) => {
  ...
  const sendStageData = async (stage, stageData) => {
    try {
      ...
      const response = await fetch('https://46.62.139.129:5678/webhook-test/profile-building', {
        method: 'POST',
        headers: {
...
const AuthScreen = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://46.62.139.129:5678/webhook/auth/${endpoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
...
const ChatInterface = ({ user, onLogout }) => {
  ...
  const fetchConversation = async (conversationId) => {
    ...
    try {
      ...
      const response = await fetch('https://46.62.139.129:5678/webhook/fetch-chat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
