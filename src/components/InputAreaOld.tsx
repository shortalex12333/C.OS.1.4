...
import { useResponsiveTheme } from '../contexts/ThemeContext';
...
export function InputArea({ onStartChat, isMobile: propIsMobile = false, isDarkMode = false, currentSearchType = 'yacht', showFAQSuggestions = true }: InputAreaProps) {
  const { 
    isDarkMode: contextIsDarkMode, 
    theme, 
    isMobile, 
    getMobileInputStyle,
  ...
  // Use context theme over prop for consistency
  const isThemeDark = contextIsDarkMode;
  const isMobileDevice = propIsMobile || isMobile;
  ...
  const searchTypes = [
    {
      ...
      placeholder: demoConfig.copy.placeholders.search
    },
    {
  ...
  const getCurrentPlaceholder = () => {
    ...
    return currentType?.placeholder || demoConfig.copy.placeholders.search;
  };
  ...
  return (
    ...
    >
      ...
      {showFAQSuggestions && showSuggestions && (
        <div style={{
          marginTop: '16px',
          marginBottom: '16px'
        }}>
          <DynamicFAQSuggestions
            onQuestionClick={(question) => {
              ...
              setTimeout(() => {
                onStartChat(question, selectedSearchType);
                setMessage('');
              }, 100);
            }}
            searchQuery={message}
      ...
      {/* Footer Disclosure */}
      <div 
        className="disclosure text-center px-4 py-2 text-xs opacity-60"
        style={{
          color: isDarkMode ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)',
          fontFamily: 'Eloquia Text, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'
        }}
      >
        {demoConfig.copy.disclosure}
      </div>
    </div>
  );
}