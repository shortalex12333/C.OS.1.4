...
// Success page template with theme detection
  ...
  try {
    ...
    if (error) {
      const errorPage = getErrorPage(
        error as string,
        error_description as string || 'Unknown error'
      );
      return res.status(400).setHeader('Content-Type', 'text/html').send(errorPage);
    }
    ...
    const userId = state as string || \`user_\${Date.now()}\`;

    // Securely store connection data with encryption
    const { connectionStore } = await import('../db/connection-store');
    ...
    const connectionId = connectionStore.storeConnection(connectionData);
    console.log(`✅ OAuth Success: Secure connection stored with ID: ${connectionId}`);

    // Return success page
    const successPage = getSuccessPage(userEmail, displayName, userId);
    return res.status(200).setHeader('Content-Type', 'text/html').send(successPage);
    ...
    return res.status(500).setHeader('Content-Type', 'text/html').send(errorPage);
  }
}